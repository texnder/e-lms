...
class Response
{
	...
	{
		// set previous url
		$this->setPreviousUrl($this->httpRequest->url."/".implode('/', $_GET));
	...
	/**
	 * set previous and current Url in session
	 *
	 * @param 	string 	$uri
	 * @return 	header
	 */
	...
	{
		// to redirect previous url set it globally..
		if (!isset($_SESSION['prev_url'])) {
			$_SESSION['prev_url'] = ($url === '') ? '/' : $url;
			$_SESSION['current_url'] = ($url === '') ? '/' : $url;
		}else{
			if ($_SESSION['prev_url'] !== $_SESSION['current_url'] && $_SESSION['current_url'] !== $url) {
